                            ...
                            <div class="btn-group">
                                <button type="button" class="btn btn-default light">
                                    ...
                                    <i class="fa fa-angle-double-left"
                                       onclick="javascript:window.location.href='/user/list?page=1&siez=2';">
                                    </i>
                                </button>
                                <button type="button" class="btn btn-default light">
                                    <%-- 上一页 --%>
                                    <i class="fa fa-chevron-left"
                                       onclick="javascript:window.location.href='/user/list?page=${ps.pageNum -1}&siez=2';">
                                    </i>
                                </button>
                                ...
                                <button type="button" class="btn btn-default light">
                                    ...
                                    <i class="fa fa-angle-double-right"
                                       onclick="javascript:window.location.href='/user/list?page=${ps.pages}&siez=2';">
                                    </i>
                                </button>
                            </div>
                        </div>
                            ...
                            <tr class="message-unread">
                                <td class="hidden-xs">
                                    <label class="option block mn">
                                        <input type="checkbox" name="mobileos" value="${user.id}">
                                ...
                                <c:choose>
                                    <c:when test="${user.identity == 3}">
                                        <td>${Contant.UserAdministrators}</td>
                                    </c:when>
                                    <c:when test="${user.identity == 2}">
                                        <td>${Contant.UserInSchool}</td>
                                    </c:when>
                                    <c:when test="${user.identity == 1}">
                                        <td>${Contant.UserOutsideSchool}</td>
                                    </c:when>
                                </c:choose>
                                <td>VIP${user.vip}</td>
                                <td>${sumList.get(user.id)}元</td>
                                <td><spring:eval expression="user.createTime"/></td>
                                <td><spring:eval expression="user.lastEditTime"/></td>
                                <td>
                                    <a href="/user/to_updateIV?id=${user.id}">编辑</a>
                                    <a href="/user/remove?id=${user.id}">删除</a>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
...
<script>
    ...
    function delall(){
        ...
        if ($("input[type='checkbox']:checked").length >0){
            ...
            javascript:window.location.href='/user/removeAll?checkId='+checkId;
        }else{
            alert("请选择你要删除的信息")
        }
    }
